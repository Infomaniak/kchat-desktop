...
release=$(curl --write-out '%{http_code}' --request POST --header "PRIVATE-TOKEN: ${GITLAB_PROJECT_ACCESS_TOKEN}" --data "name=Mac-arm64.dmg&url=https://download.storage5.infomaniak.com/kchat/kchat-desktop-${GITLAB_PROJECT_TAG}-mac-arm64.dmg" "https://gitlab.infomaniak.ch/api/v4/projects/3236/releases/${GITLAB_PROJECT_TAG}/assets/links")
